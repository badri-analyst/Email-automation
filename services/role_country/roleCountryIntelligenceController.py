"""Role-country intelligence controller — AI-first, deterministic fallback."""
...
from services.ai import roleCountryAiService
...
class RoleCountryIntelligenceController:
    ...
    def _build_once(self, request: RoleCountryInput) -> RoleCountryOutput:
        """Build role-country intelligence — AI first, deterministic fallback."""
        ...
        # --- AI-first path ---
        ai_result = roleCountryAiService.enhance(request.model_dump())
        if ai_result:
            return RoleCountryOutput(
                ...
                role_country_reason="Role-country intelligence generated by AI.",
                intelligence_source_type="configured_role_country_rule",
                role_summary=ai_result.get("role_summary", INSUFFICIENT_DATA),
                core_responsibilities=ai_result.get("core_responsibilities", []),
                country_role_expectations=ai_result.get("country_role_expectations", [INSUFFICIENT_DATA]),
                priority_skills=ai_result.get("priority_skills", []),
                business_keywords=ai_result.get("business_keywords", []),
                email_positioning_angle=ai_result.get("email_positioning_angle", INSUFFICIENT_DATA),
                country_specific_email_tone=ai_result.get("country_specific_email_tone", INSUFFICIENT_DATA),
                proof_points_to_use=ai_result.get("proof_points_to_use", []),
                things_to_avoid=ai_result.get("things_to_avoid", []),
                personalization_guidance=ai_result.get("email_positioning_angle", INSUFFICIENT_DATA),
            )
